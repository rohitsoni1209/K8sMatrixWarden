...
<p align="center">
  ...
  <em>MITRE ATT&CK-Aligned · Domain-Sharded Scanning · Runtime Protection · Intelligent Remediation</em>
</p>
...
> v3.0 supersedes the v2.0 Architecture & Design Document. The macro-architecture (Orchestrator · Scanner · Runtime · Remediation · MCP Server) is unchanged. **The change is internal to the Scanner Agent**, which is re-architected from a fixed set of "8 modules" into **Domain-Sharded Execution with a Cross-Cutting Rule/Technique Registry**, fully aligned to the MITRE ATT&CK for Kubernetes / Containers threat model.

| Area | v2.0 | v3.0 |
|---|---|---|
| **Scanner structure** | 8 monolithic modules, each a single scan function | 10 **domain shards** (8 existing + Admission Control + Cloud IAM), each a plugin owning a set of independent **Rules** |
| **Unit of detection** | Module-level check | **Rule** — one technique-level check with self-declared taxonomy tags |
| **MITRE mapping** | Free-text, mapped after the fact | Declarative per-rule tags, indexed by a **MITRE Mapping Engine**, validated in CI against **ATT&CK for Containers** IDs |
| **Threat matrix** | 10 columns (incl. a non-existent "Collection" tactic) | Corrected to the real **9 tactics** of the Kubernetes Threat Matrix |
| **Scan selection** | "Scan namespace / cluster" | Scan by **scope** (cluster→pod→image) **× selector** (tactic · technique · module · rule · composite alias · framework) |
| **Execution** | Run whole module | Registry **resolves selector → rule_id set**, one shared evidence fetch, parallel rule execution |
| **Extensibility** | Add a module (code) | Register a **plugin**, tag a rule — additive, no engine change |
| **RBAC** | One blanket `k8s-security-reader` role | Per-plugin **scoped RoleBindings** minted from each shard's declared verbs |
...
| 9 | [Remediation Agent](#9-remediation-agent) | Auto-fix engine with rollback |
...
| 13 | [OWASP Kubernetes Top 10 (2025)](#13-owasp-kubernetes-top-10-2025) | Complete coverage with detection & remediation |
...
**K8s Security Tool** is an AI-powered, Kubernetes-native security platform that continuously scans, monitors, and secures Kubernetes clusters. It uses a **multi-agent architecture** where specialized agents collaborate to detect vulnerabilities, map attack surfaces, enforce compliance, and auto-remediate security issues.

In v3.0, the Scanner Agent is organized around the **MITRE ATT&CK threat model for Kubernetes**: every check is a **Rule** tagged with the tactic and technique it detects, so users can scan by resource *or* by adversary behavior ("scan for Persistence", "scan only Container Escape") through one uniform engine.

### Why

Kubernetes clusters are complex, dynamic, and present a massive attack surface. Misconfigurations are the #1 cause of K8s breaches. Manual security reviews don't scale. Existing tools scan but don't remediate, and they report findings without connecting them to how an attacker actually operates. This tool closes the loop and speaks the attacker's language: **Detect → Analyze (map to ATT&CK) → Fix → Verify**.
...
│              │     │                 │     │  (with user    │     │              │
│  Rules run   │     │  Map to ATT&CK, │     │   approval)    │     │  Re-scan to  │
│  by shard,   │     │  OWASP, CIS.    │     │                │     │  confirm fix │
│  resolved by │     │  Score risk +   │     │  Apply patches,│     │              │
│  tactic/tech │     │  attack-path.   │     │  generate      │     │  Score       │
│  or resource │     │  Prioritize.    │     │  policies      │     │  improved?   │
│              │     │                 │     │                │     │              │
└──────────────┘     └─────────────────┘     └────────────────┘     └──────────────┘
```
...
| 🗺️ **Attack Surface Mapping** | Enumerates every external entry point, container escape vector, privilege escalation path, and lateral movement route |
...
| 🛡️ **Runtime Threat Detection** | Real-time syscall monitoring (Falco/Tetragon), behavioral baselines, drift detection, K8s audit analysis |
| 🔧 **Intelligent Remediation** | AI-generated fixes with exact `kubectl` commands shown before execution, rollback on failure, full audit trail |
...
|---|---|
| **Scan-only, no remediation** | Auto-remediation engine with human-in-the-loop approval |
...
│   │  AGENT          │ │  AGENT         │ │  AGENT           │                      │
│   │                 │ │ (DaemonSet)    │ │                  │                      │
│   │ Registry Layer: │ │ Falco/Tetragon │ │ Auto-Fix Engine  │                      │
│   │ • Scanner Reg.  │ │ Audit Analyzer │ │ Policy Generator │                      │
│   │ • Rule Registry │ │ Drift Detector │ │ Rollback Ctrl    │                      │
│   │ • MITRE Mapping │ │ Baseline Engine│ │ Audit Logger     │                      │
...
│   │  kubectl cmds · scan-tool cmds · remediation playbooks ·          │            │
│   │  CVE KB · compliance rules · 🆕 MITRE/OWASP/CIS taxonomy files    │            │
│   └────────────────────────────────┬─────────────────────────────────┘            │
│                                    ▼                                               │
│   ┌──────────────────────────────────────────────────────────────────┐            │
...
| **🛡️ Runtime** | Real-time monitoring — syscall, audit events, drift, behavioral anomalies (same rule/registry pattern) | DaemonSet | 1 per node |
...
│  │ Evidence Collector │◄───────────────────────────────▶│ ☸️ K8s API / MCP /  │   │
...
User Request
   │
   ▼  🧠 Orchestrator: [1] classify intent → [2] extract scope → [3] resolve selector
   ▼  ScanRequest { scope, selector[], mode, output }  →  [5] confirm resolved rule set
   │
   ▼  🔍 Scanner: Registry.resolve(selector) → rule_id set (may span shards)
   ▼           Evidence Collector.fetch(union of needs ∩ scope) → shared snapshot
   ▼           Detection Engine → raw findings (each tagged)
   │
   ▼  📦 MCP Server consulted for commands / playbooks / taxonomy during the above
   ▼  ☸️ K8s Cluster / Cloud IAM API queried by the Evidence Collector
   │
   ▼  🔍 Result Aggregator (dedupe + merge tags) → Risk Scoring → Reporting
   ▼  🧠 Orchestrator: aggregate → map summary → ask confirmation (if remediation)
   │
   ▼  📊 Report to User (tag-filterable)
```
...
| 6 | **Human-in-the-Loop** | All destructive actions require explicit user confirmation |
| 7 | **Least Privilege** | Read-only by default; write scoped to Remediation Agent; **per-plugin scoped RBAC** below that |
...
| 10 | **Idempotent** | Same scan twice → same result; fixes safe to re-apply |
...
The Orchestrator is the **single entry point**. It never scans or fixes directly — it compiles user intent into a `ScanRequest` and delegates.

```
User Request
   │
   ▼
[1] INTENT CLASSIFICATION   → SCAN · AUDIT · MAP · MONITOR · INVESTIGATE · REMEDIATE · REPORT · DOWNLOAD
   │
   ▼
[2] SCOPE EXTRACTION        → cluster | namespace | workload | node | pod | image | helm_release
   │
   ▼
[3] SELECTOR RESOLUTION     → tactic · technique · module/domain · rule_id · composite alias · framework
   │                          (matched against the Rule Registry's term index — see §6/§7)
   ▼
[4] BUILD ScanRequest       → { scope, selector[], mode: sync|async, output: md|json|... }
   │
   ▼
[5] CONFIRMATION            → "This resolves to N rules under <X>: [list]. Proceed?"
   │
   ▼
Route to Scanner / Runtime / Remediation → aggregate → map → score → report
```
...
| Rule | Enforcement |
|---|---|
| No auto-remediation without confirmation | Orchestrator blocks Remediation Agent until user confirms |
| Show exact changes before applying | Remediation must output the exact `kubectl patch` / YAML diff first |
| Confirm resolved rule set before scanning | For tactic/alias scans, show the resolved rule list before executing (`[5]` above) |
| Rollback on failure | Remediation snapshots state before change; auto-rollback if health check fails |
| Audit trail for every action | Every scan, finding, and remediation logged with timestamp, user, agent ID |
| Read-only by default | Scanner/Runtime use read-only roles; per-plugin scoped (see §20) |
| Scoped write access | Only Remediation uses `k8s-security-remediator` — and only when triggered |
...
The atomic unit of detection is a **Rule** — one technique-level check that self-declares its taxonomy. A **domain shard** is a plugin that owns a set of rules sharing an evidence-fetch pattern (not a single scan function as in v2.0).
...
remediation_ref: playbook/pod-security-context
evidence_needs: [Pod]
```
...
> **📌 Correction from earlier drafts:** the CIS Kubernetes Benchmark v1.8 has **130 controls**, not 242. The "242" figure was an over-count. The authoritative source is kube-bench's `cis-1.8` config; the verified section breakdown is below.
...
```bash
k8ssec cis --mock                              # full 130-control benchmark
k8ssec cis --kube-bench-json kb.json           # resolve the 31 node file controls
k8ssec cis --profile eks                        # managed cluster: control plane → N/A
k8ssec cis -o markdown --output-file cis.md    # export the compliance report
k8ssec cis --fail-on-fail                       # CI: exit 1 on any FAIL
```

The full catalog lives in code as data (`k8ssec/frameworks/cis_catalog.py`), so adding CIS v1.9/1.10 or a managed-provider variant is a new catalog file, not an engine change.

#### 5.9.2 Mitigating the node-access limitation — how NEEDS_NODE shrank from 69 → 31
...
Closes the Initial Access / Privilege Escalation / Lateral Movement "cloud credentials & access cloud resources" gaps — the highest-leverage v2.0 gap (touches 3 tactics).
...
| **Rule / Technique Registry** | Catalog of every rule: `id`, `title`, `owning_shard`, `resource_scope`, `severity`, `detection_method`, `mitre[]`, `owasp`, `cis[]`, `nsa_cisa[]`, `remediation_ref`, `evidence_needs` |
...
```
1. Orchestrator → ScanRequest { scope, selector[] }
2. Registry.resolve(selector) → concrete rule_id set   (may span many shards)
3. Evidence Collector.fetch(∪ evidence_needs ∩ scope) → one shared snapshot
4. Detection Engine → execute each rule → raw findings (each tagged)
5. Result Aggregator → dedupe + merge taxonomy tags
6. Risk Scoring → severity × exploitability × blast radius + attack-path bonus
7. Reporting → render in requested format, filterable by any tag axis
```
...
User text
  │
  ▼ [1] Coarse intent   → SCAN | AUDIT | MAP | MONITOR | REMEDIATE …
  ▼ [2] Scope extract   → cluster / namespace / workload / node / pod / image / helm
  ▼ [3] Selector resolve against the Rule Registry term index:
        exact/fuzzy match → tactic | technique | module | composite alias
  ▼ [4] Ambiguity → LLM reasoning over the SAME term index (generated from rule
        titles/tags, so it never drifts from what actually exists)
  ▼ [5] Confirm: "resolves to N rules under <X>: [list]. Proceed?"
  ▼ execute
```
...
Same as v2.0: 48h training → anomaly detection (unexpected processes, new connections, CPU/mem spikes), new-binary/new-port/config-tamper drift detection. Drift rules are registered and tagged (e.g. `runtime-backdoor-container` → Persistence; CoreDNS ConfigMap modification → Lateral Movement).
...
|---|---|
| **Pod Security** | `kubectl patch deploy <n> -p '{"spec":{"template":{"spec":{"securityContext":{"runAsNonRoot":true}}}}}'` |
| **RBAC** | `kubectl patch sa default -n <ns> -p '{"automountServiceAccountToken":false}'` |
| **Network Policy** | `kubectl apply -f default-deny-ingress.yaml` |
| **Secrets** | Rotate, migrate env→volume, enable etcd encryption |
| **Images** | `kubectl set image deploy/<n> <c>=<img>@sha256:<digest>` |
| **Compliance** | `kubectl label ns <ns> pod-security.kubernetes.io/enforce=restricted` |
| **Admission** 🆕 | Remove/scope malicious webhook, set `failurePolicy: Fail` |
| **Cloud IAM** 🆕 | Recommend scoped IRSA/WI policy (advisory — cloud-side change) |

### 9.3 Rollback Lifecycle

```
① SNAPSHOT  → kubectl get <res> -o yaml > /snapshots/
② SHOW DIFF → exact change displayed to user
③ CONFIRM   → N aborts, no change
④ APPLY     → kubectl patch / apply
⑤ HEALTH    → wait for rollout, verify Running+Ready
⑥ ON FAIL   → kubectl apply -f /snapshots/<snap>.yaml → "rolled back"
⑦ AUDIT LOG → timestamp, user, finding, fix, result
```
...
The knowledge layer. All agents query it for commands, scan logic, fix procedures, and now **taxonomy**. It has **6 datasets**:

| # | Dataset | Contents | Used By |
|---|---------|----------|---------|
| 📘 1 | **kubectl Security Commands** | 60+ inspection commands | Scanner |
| 📗 2 | **Scanning Tool Commands** | Trivy, kube-bench, kubeaudit, kubesec, kubescape, popeye | Scanner |
| 📙 3 | **Remediation Playbooks** | patch templates, YAML fixes, policy templates | Remediation |
| 📕 4 | **CVE Knowledge Base** | 200+ K8s CVEs with detection & fix versions | Scanner + Orchestrator |
| 📒 5 | **Compliance Rule Sets** | CIS rules, PSS, NSA/CISA controls | Scanner |
| 🆕 📓 6 | **Taxonomy Files** | ATT&CK-for-Containers IDs, OWASP K8s Top 10, CIS, NSA, redguard aliases | MITRE Mapping Engine |

Dataset 6 is what the Mapping Engine validates rule tags against, and what lets the whole platform pin/upgrade a taxonomy version without a code change.
...
> **📌 Correction from v2.0:** The Kubernetes Threat Matrix (Microsoft original, extended by Redguard) has **9 tactics**, not 10. v2.0's "Collection" column does not exist in the K8s-specific matrix — it was borrowed from the general Enterprise ATT&CK matrix. It has been removed. The machine-readable mapping uses **MITRE ATT&CK for Containers** technique IDs (`T16xx`); Redguard names are display aliases only.
...
### 12.2 Detection Coverage (post-v3.0)
...
**Coverage vs. v2.0:** the 8 true gaps (Cloud IAM, Admission Control, CoreDNS RBAC, CronJobs, sshd, kubeconfig, proxy source-IP, CNI anti-spoof) are closed by shards ⑨ & ⑩ plus the 🆕 rules above.

---

## 13. OWASP Kubernetes Top 10 (2025)

Every OWASP category maps to shards/rules (unchanged in substance from v2.0; now expressed as rule tags `owasp: Kxx`).
...
```yaml
# ═══════════════════════════════════════════════════
# K8s Security Tool — Scanner Configuration v3.0
# ═══════════════════════════════════════════════════
version: "3.0"
...
scanner:
  ...
  custom_rule_paths: ["/etc/k8ssec/custom-rules/"]
```
...
🧠 ScanRequest{ scope: cluster }  → registry resolves ALL scope-matching rules
🔍 rules run across 10 shards, one shared evidence fetch
📊 🔴 19 CRITICAL · 🟠 44 HIGH · 🟡 22 MEDIUM · Score 31/100 (CRITICAL)
   MITRE: 28/40 techniques applicable · Auto-remediate CRITICAL? (Y/N)
```

### Workflow 2 — Scan by Tactic 🆕
```
👤 "Scan production for Persistence"
🧠 ScanRequest{ scope:{ns:production}, selector:[{tactic:Persistence}] }
🔍 resolve → { hostpath-writable, hostpath-root, cronjob-suspicious,
...
📊 2 findings: writable hostPath in batch-worker (also enables PrivEsc+Lateral → attack-path bonus),
   1 suspicious CronJob. Fix? (Y/N)
```

### Workflow 3 — Scan by Technique / Alias 🆕
```
👤 "Scan only for Container Escape"
🧠 selector:[{technique:"Container Escape"}] → alias expands to 4 rules
📊 3 escape vectors: 2 privileged, 1 docker.sock. Ranked by blast radius.
```
...
🧠 Intent=MAP → shard ⑧
📊 Entry points 8 · Escape vectors 5 · PrivEsc paths 4 · Lateral routes 6 · Risk 7.8/10
```

### Workflow 5 — Runtime Threat Response
```
🛡️ Falco: shell spawned in web-app (prod) → curl 169.254.169.254
🧠 auto-routes to Scanner (investigate) + Runtime (details)
📊 CRITICAL: shell + metadata API = credential theft
   MITRE: Execution → bash | Credential Access → Managed Identity
   Execute emergency response (kill pod, block IMDS, rotate token)? (Y/N) → Y
🔧 ✅ pod killed ✅ NetworkPolicy applied ✅ token rotated ✅ audit logged
```

### Workflow 6 — Compliance Audit (full CIS Benchmark Engine)
```
👤 "Run CIS benchmark audit"
🧠 Intent=AUDIT → CIS Benchmark Engine evaluates all 130 controls (§5.9.1–5.9.2)
📊 CIS Kubernetes Benchmark v1.8 — 130 controls  (profile: self-managed)
   ✅ PASS 14  ❌ FAIL 51  🔶 MANUAL 34  ➖ NA 0  ⚙️ NEEDS_NODE 31
   Automated pass rate 22% · auto-evaluated coverage 50%
   §1 Control Plane: ❌22 (profiling on, AlwaysAllow authz, weak admission set, …)
   §5 Policies: ❌18  (privileged pods, wildcard RBAC, no NetworkPolicies, …)
   Supply `--kube-bench-json` to resolve the 31 remaining node file controls.
   Auto-fix the FAILed controls? (Y/N)
```
...
🧠 Intent=DOWNLOAD, format=markdown → Report Store
✅ k8s-security-report-20260715.md (85 KB) — CLI/API/kubectl cp/Web UI
```
...
| 8.1–10.0 | 🔴 Critical | Emergency remediation |

### 18.2 Report Formats

| Format | Download | Use Case |
|---|---|---|
| Terminal | `k8ssec report show` | Interactive scans |
| Markdown | `k8ssec report download --format markdown` | Docs, sharing |
...
**New in v3.0 — tag-based filtering is first-class** because tags are finding metadata, not agent identity:
```bash
k8ssec report download --format markdown --tactic Persistence
k8ssec report download --format json --owasp K01
k8ssec report download --format sarif --cis 5.2.1
k8ssec report download --format html --shard rbac_identity
```
...
└──────────────────────────┘  └──────────────────────────┘
┌──────────────────────────┐  ┌──────────────────────────┐
│ remediation-agent (1)    │  │ k8s-security-mcp (STS 1)  │
└──────────────────────────┘  └──────────────────────────┘
┌──────────────────────────┐  ┌──────────────────────────┐
│ security-dashboard (1)    │  │ report-store (STS + PVC) │
└──────────────────────────┘  └──────────────────────────┘
┌──────────────────────────────────────────────────────────┐
...
│ compliance trends · remediation audit · baselines ·       │
│ custom rule plugins · generated reports                   │
└──────────────────────────────────────────────────────────┘
```

**Resource Requirements** (unchanged from v2.0):
...
v3.0 keeps the two-tier model (read-only vs. remediator) **and** adds **per-plugin scoped RoleBindings** minted by the Plugin Loader from each shard's declared verbs — so a compromised/buggy shard is blast-radius-limited to what it declared, not to everything the Scanner can read.
...
---
# ── Tier 2: write (Remediation Agent ONLY, only when triggered) ──
apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata: { name: k8s-security-remediator }
rules:
  - { apiGroups: [""], resources: [pods, services, secrets, configmaps, serviceaccounts], verbs: [get, list, watch, update, patch, delete] }
  - { apiGroups: [apps], resources: [deployments, daemonsets, statefulsets], verbs: [get, list, watch, update, patch] }
  ...
  - { apiGroups: [admissionregistration.k8s.io],
      resources: [mutatingwebhookconfigurations, validatingwebhookconfigurations], verbs: [get, list, watch, update, patch, delete] }

---
# ── Tier 3 (NEW): per-plugin scoped role, generated from the shard manifest ──
...
| Policy Engine | [Kyverno](https://github.com/kyverno/kyverno) / [OPA Gatekeeper](https://github.com/open-policy-agent/gatekeeper) | Shard ⑦⑨ + Remediation |
...
- Remediation Agent (confirm + rollback) · MCP Dataset 3 🔲
...
<p align="center">
  <strong>K8s Security Tool</strong> — v3.0<br/>
