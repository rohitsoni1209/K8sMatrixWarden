...
def ref(ev: Evidence, res: dict):
    ...
    Pod's direct owner is a Job — neither is the object remediation should ever target
    (patching a Deployment-owned ReplicaSet's template doesn't update its running Pods;
    a Job's template is effectively immutable in practice). So resolve one hop further
    when the intermediate object is present in the evidence already fetched for this
    scan, and prefer the resolved *owner's* own labels/annotations over the Pod's own
    for Helm/ArgoCD/Flux detection (that's where those tools actually stamp their
    markers). If the hop can't be confirmed from evidence, the direct (unresolved) owner
    is kept as-is — the remediation engine treats that conservatively (no guessing)."""
    r = _base_ref(res)
    ...
    def rules(self):
        ...
        return [
            ...
            Rule("workload-host-pid", "Host PID namespace shared", self.name, ["Pod"],
                 ...
                 remediation_ref="playbook/disable-host-pid"),
            Rule("workload-host-network", "Host network shared", self.name, ["Pod"],
                 ...
                 remediation_ref="playbook/disable-host-network"),
            Rule("workload-host-ipc", "Host IPC namespace shared", self.name, ["Pod"],
                 ...
                 remediation_ref="playbook/disable-host-ipc"),
