...
BANNER = r"""
  🛡️  k8ssec chat — MITRE ATT&CK-aligned Kubernetes security assistant
...
class ChatSession:
    ...
    def _followup(self, raw: str, low: str) -> Optional[str]:
        ...
        if _any(low, r"\b(export|save|download|render|write)\b") and \
                _any(low, r"\b(markdown|md|json|html|sarif|text|report)\b"):
            fm = re.search(r"\b(markdown|md|json|html|sarif|text)\b", low)
    ...
    def _export(self, fmt: str, filename: Optional[str] = None) -> str:
        ...
        ext = {"markdown": "md", "json": "json", "html": "html", "sarif": "sarif",
               "text": "txt"}.get(fmt, "txt")
        out = self.platform.reporting.render(self.last_result, fmt)
        path = filename or f"k8ssec-report-{self.last_result.scan_id}.{ext}"
        try:
