"""
Reporting Engine (§18.2) — rich, multi-format security reports.

Renders a ScanResult in six formats, each designed to be genuinely useful rather than a
flat dump:
  ...
  markdown  — the flagship: frontmatter, verdict, risk gauge, coverage dashboards,
              per-finding cards with EMBEDDED kubectl fix commands, a prioritized
              remediation plan, and an appendix
  json      — full structured data + a summary block + per-finding remediation commands
  sarif     — SARIF 2.1 with security-severity, tags, help text, fingerprints (GitHub-ready)
  html      — self-contained, responsive, dark/light card report
...
class ReportingEngine:
    def render(self, result: ScanResult, fmt: str = "terminal") -> str:
    ...
    def markdown(self, result: ScanResult) -> str:
        ...
        md += [
            ...
            f'tool: "k8ssec {result.tool_version}"',
            f'mode: "{result.mode}"',
            f'scope: "{result.request.scope.describe()}"',
            f'selector: "{result.request.selector.describe()}"',
        ...
        md += [
            ...
            "2. [Coverage & Exposure](#coverage)",
            "3. [Findings](#findings)"
            + (" · [⚡ Attack-Path Amplified](#attackpaths)" if multi else ""),
            "4. [Prioritized Remediation Plan](#remediation)",
            "5. [Appendix](#appendix)",
            "",
            "---",
            "",
        ...
        # ---- coverage --------------------------------------------------- #
        md += ["<a id=\"coverage\"></a>", "## 2. 🎯 Coverage & Exposure", ""]
        ...
        md += [
            ...
            f"| Generated | {result.generated_at} |",
            f"| Tool version | k8ssec {result.tool_version} |",
            f"| Mode | {result.mode} |",
            f"| Scope | {result.request.scope.describe()} |",
            f"| Selector | {result.request.selector.describe()} |",
            ...
            "",
            "---",
            f"<sub>Generated by 🛡️ k8ssec {result.tool_version} — "
    ...
    def json(self, result: ScanResult) -> str:
        ...
        # attach concrete remediation commands to each finding
        for fd, f in zip(doc["findings"], result.findings):
            title, cmds = _fix_commands(f)
            fd["remediation"] = {"title": title, "commands": cmds}
    ...
    def sarif(self, result: ScanResult) -> str:
        ...
        for f in result.findings:
            ...
            if f.rule_id not in rules:
                rules[f.rule_id] = {
                    ...
                    "helpUri": "https://owasp.org/www-project-kubernetes-top-ten/",
                    "help": {"text": _fix_text(f)},
            ...
            results.append({
                ...
                "partialFingerprints": {
                    "k8ssec/v1": f"{f.rule_id}:{f.resource.namespace}:"
        ...
        doc = {
            ...
            "runs": [{
                "tool": {"driver": {
                    "name": "k8ssec",
                    "fullName": "k8ssec — MITRE ATT&CK-aligned Kubernetes scanner",
    ...
    def html(self, result: ScanResult) -> str:
        ...
        cards = []
        for f in findings:
            title2, cmds = _fix_commands(f)
            fix_html = ""
            if cmds:
                joined = "\n".join(_esc(cm) for cm in cmds)
                fix_html = (f"<details><summary>🛠️ Remediation — {_esc(title2 or '')}"
                            f"</summary><pre class='cmd'>{joined}</pre></details>")
            mitre = " ".join(f"<span class='tag mitre'>{_esc(m.tactic.value)} · "
                             f"{m.technique_id}</span>" for m in f.mitre)
            tags = ""
            if f.owasp:
                tags += f"<span class='tag owasp'>{f.owasp}</span>"
            for cc in f.cis:
                tags += f"<span class='tag cis'>CIS {cc}</span>"
            ...
            cards.append(
                ...
                f"<div class='cmsg'>{_esc(f.message)}</div>"
                f"<div class='tags'>{mitre}{tags}"
...
def _finding_card(f: Finding, i: int) -> list[str]:
    ...
    out = [
        ...
        f"| **MITRE ATT&CK** | {_mitre_long(f) or '—'} |",
        f"| **OWASP** | {_owasp_label(f.owasp)} |",
        f"| **CIS** | {', '.join(f.cis) or '—'} |",
        f"| **NSA/CISA** | {', '.join(f.nsa_cisa) or '—'} |",
    ...
    title, cmds = _fix_commands(f)
    if cmds:
        out += ["<details><summary>🛠️ <strong>Remediation</strong> — "
                f"{title}</summary>", "", "```bash"]
        out += cmds
        out += ["```", "</details>", ""]
...
# -- fix commands (lazy import to keep core independent of mcp) ---------- #
def _fix_commands(f: Finding):
    if not f.remediation_ref:
        return None, []
    try:
        from ..mcp.datasets import PLAYBOOKS
    except Exception:
        return None, []
    pb = PLAYBOOKS.get(f.remediation_ref)
    if not pb:
        return None, []
    res = f.resource
    ns = res.namespace or "default"
    kind = (res.kind or "deployment").lower()
    cmds = []
    for c in pb.get("commands", []):
        try:
            cmds.append(c.format(name=res.name, namespace=ns, kind=kind))
        except Exception:
            cmds.append(c)
    return pb.get("title"), cmds
...
.cmsg{margin:.5rem 0;color:var(--fg)}
...
{cards}
<footer>Generated by 🛡️ k8ssec — MITRE ATT&amp;CK-aligned Kubernetes security scanner.</footer>
