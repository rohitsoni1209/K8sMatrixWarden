    ...
    for res, _spec, containers in _iter(ev):
        for c in containers:
            if Evidence.dig(c, "securityContext.privileged") is True:
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' runs privileged "
                    f"(full host access)",
    ...
    for res, spec, containers in _iter(ev):
        ...
        for c in containers:
            ...
            if uid == 0 or (effective_nonroot is not True and uid in (None, 0)):
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' may run as root "
    ...
    for res, _spec, containers in _iter(ev):
        for c in containers:
            ...
            if ape is True or ape is None:
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' allows privilege escalation "
    ...
    for res, _spec, containers in _iter(ev):
        for c in containers:
            if Evidence.dig(c, "securityContext.readOnlyRootFilesystem") is not True:
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' has a writable root "
...
def _dangerous_caps(rule, ev, scope):
    for res, _spec, containers in _iter(ev):
        for c in containers:
            ...
            if hit:
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' adds dangerous "
    ...
    for res, _spec, containers in _iter(ev):
        for c in containers:
            ...
            if "ALL" not in dropped:
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' does not drop ALL "
                    f"capabilities",
    ...
    def _check(rule, ev, scope):
        for res, spec, _c in _iter(ev):
            if spec.get(field) is True:
                yield rule.finding(
                    ref(res), f"workload shares host {field} namespace",
    ...
    for res, spec, _c in _iter(ev):
        for vol in spec.get("volumes", []) or []:
            ...
            if "docker.sock" in path:
                yield rule.finding(
                    ref(res), f"mounts the Docker socket ({path}) — container escape",
    ...
    for res, spec, _c in _iter(ev):
        for vol in spec.get("volumes", []) or []:
            ...
            if path == "/" or path in ("/etc", "/var/run", "/etc/kubernetes/manifests"):
                yield rule.finding(
                    ref(res), f"mounts sensitive hostPath '{path}' — full/host-critical "
                    f"filesystem access",
                    blast_radius=BlastRadius.CLUSTER,
                    evidence={"volume": vol.get("name"), "path": path})
    ...
    for res, spec, _c in _iter(ev):
        for vol in spec.get("volumes", []) or []:
            ...
            if hp and path not in (None, "/") and path not in (
                ...
                yield rule.finding(
                    ref(res), f"mounts a writable hostPath '{path}' (persistence / "
...
def _missing_limits(rule, ev, scope):
    for res, _spec, containers in _iter(ev):
        for c in containers:
            ...
            if not limits:
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' has no resource limits "
                    f"(DoS amplification)",
                    evidence={"container": c.get("name")})
    ...
    for res, spec, containers in _iter(ev):
        ...
        for c in containers:
            ...
            if not (c_seccomp or pod_seccomp):
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' has no seccomp profile",
                    evidence={"container": c.get("name")})
...
def _sa_automount(rule, ev, scope):
    for res, spec, _c in _iter(ev):
        if spec.get("automountServiceAccountToken") is not False:
            yield rule.finding(
                ref(res), "service-account token is auto-mounted "
    ...
    for res, _spec, containers in _iter(ev):
        for c in containers:
            ...
            if image.endswith(":latest") or (":" not in image.split("/")[-1]
                                             and "@" not in image):
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' uses a mutable/unpinned "
                    f"image tag: {image}",
    ...
    for res, _spec, containers in _iter(ev):
        for c in containers:
            ...
            if "sshd" in cmd or "sshd" in (c.get("image", "")):
                yield rule.finding(
                    ref(res), f"container '{c.get('name')}' appears to run an SSH server",
                    evidence={"container": c.get("name")})
    ...
    def rules(self):
        ...
        return [
            ...
            Rule("workload-run-as-root", "Container runs as root", self.name,
                 ...
                 mitre=[M(T.PRIVILEGE_ESCALATION, "T1610", "Deploy Container")],
                 owasp="K01", cis=["5.2.7"], evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/run-as-non-root"),
            Rule("workload-allow-priv-escalation", "Privilege escalation allowed",
                 ...
                 owasp="K01", cis=["5.2.6"], evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/pod-security-context"),
            Rule("workload-writable-root-fs", "Writable root filesystem", self.name,
                 ...
                 owasp="K01", evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/read-only-root-fs"),
            Rule("workload-dangerous-caps", "Dangerous capabilities added", self.name,
                 ...
                 owasp="K01", cis=["5.2.8", "5.2.9"], evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/drop-capabilities"),
            Rule("workload-caps-not-dropped", "Capabilities not dropped (ALL)", self.name,
                 ["Pod"], S.HIGH, DM.STATIC_CONFIG, _caps_not_dropped,
                 mitre=[M(T.PRIVILEGE_ESCALATION, "T1610", "Deploy Container")],
                 owasp="K01", cis=["5.2.9"], evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/drop-capabilities"),
            Rule("workload-host-pid", "Host PID namespace shared", self.name, ["Pod"],
                 ...
                 mitre=[M(T.PRIVILEGE_ESCALATION, "T1610", "Deploy Container")],
                 owasp="K01", cis=["5.2.3"], evidence_needs=WORKLOAD_KINDS),
            Rule("workload-host-network", "Host network shared", self.name, ["Pod"],
                 ...
                 owasp="K01", cis=["5.2.5"], evidence_needs=WORKLOAD_KINDS),
            Rule("workload-host-ipc", "Host IPC namespace shared", self.name, ["Pod"],
                 ...
                 mitre=[M(T.PRIVILEGE_ESCALATION, "T1610", "Deploy Container")],
                 owasp="K01", cis=["5.2.4"], evidence_needs=WORKLOAD_KINDS),
            Rule("workload-docker-socket", "Docker socket mounted", self.name, ["Pod"],
                 ...
                 owasp="K01", nsa_cisa=["Pod Security"], evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/remove-hostpath"),
            Rule("workload-hostpath-root", "Sensitive hostPath mount", self.name, ["Pod"],
                 ...
                 mitre=[M(T.PERSISTENCE, "T1610", "Deploy Container"),
                        ...
                        M(T.LATERAL_MOVEMENT, "T1610", "Deploy Container")],
                 owasp="K01", cis=["5.2.12"], evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/remove-hostpath"),
            ...
            Rule("workload-missing-limits", "Missing resource limits", self.name, ["Pod"],
                 ...
                 owasp="K01", evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/resource-limits"),
            Rule("workload-no-seccomp", "No seccomp profile", self.name, ["Pod"],
                 ...
                 owasp="K01", cis=["5.7.2"], evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/pod-security-context"),
            Rule("workload-sa-token-automount", "SA token auto-mounted", self.name,
                 ["Pod"], S.MEDIUM, DM.STATIC_CONFIG, _sa_automount,
                 mitre=[M(T.CREDENTIAL_ACCESS, "T1528", "Steal Application Access Token")],
                 owasp="K03", cis=["5.1.6"], evidence_needs=WORKLOAD_KINDS,
                 remediation_ref="playbook/disable-sa-automount"),
            Rule("workload-latest-tag", "Mutable :latest image tag", self.name, ["Pod"],
                 ...
                 remediation_ref="playbook/pin-image-digest"),
