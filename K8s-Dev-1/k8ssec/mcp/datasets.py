...
K8s Security MCP Server — the 6 knowledge datasets (§10).
  ...
  6. Taxonomy Files  (loaded from k8ssec/taxonomy via bootstrap)
...
PLAYBOOKS = {
    "playbook/pod-security-context": {
        "title": "Harden pod securityContext",
        "commands": [
            "kubectl patch {kind} {name} -n {namespace} -p '{{\"spec\":{{\"template\":"
    ...
    "playbook/drop-capabilities": {
        "title": "Drop ALL capabilities",
        "commands": [
            "kubectl patch {kind} {name} -n {namespace} --type=json -p "
            "'[{{\"op\":\"add\",\"path\":\"/spec/template/spec/containers/0/"
            "securityContext/capabilities\",\"value\":{{\"drop\":[\"ALL\"]}}}}]'",
        ],
    },
    "playbook/read-only-root-fs": {
        "title": "Set readOnlyRootFilesystem",
        "commands": [
            "kubectl patch {kind} {name} -n {namespace} --type=json -p "
            "'[{{\"op\":\"add\",\"path\":\"/spec/template/spec/containers/0/"
    ...
    "playbook/disable-sa-automount": {
        "title": "Disable SA token automount",
        "commands": [
            "kubectl patch sa {name} -n {namespace} "
